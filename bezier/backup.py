...
def bezier_interpolate_2d(points):
    n = len(points)-1
...
def bezier_interpolate_thomas(points):
    n = len(points)-1
    ...
    Ax = thomas_solve(diagonals,Px)
    Ay = thomas_solve(diagonals,Py)
    A = np.r_[[Ax],[Ay]].T
...
def bezier_interpolate(points):
    bezier_interpolate_thomas(points)
    n = len(points) - 1
...
def get_bezier_cubic(points):
    A, B = bezier_interpolate_2d(points)
